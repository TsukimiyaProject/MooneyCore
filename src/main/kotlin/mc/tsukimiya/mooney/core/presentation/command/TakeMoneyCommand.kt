...
class TakeMoneyCommand : TabExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        val reason = if (args.size > 2) args[2] else MooneyCore.formatter.formatMessage("log.take", executor)
        try {
            MooneyCore.api.withdrawMoney(target, amount, reason)
            sender.sendMessage(MooneyCore.formatter.formatMessage("message.takemoney.success", args[0], args[1]))
        } catch (e: MoneyOverLowerLimitException) {
            sender.sendMessage(MooneyCore.formatter.formatMessage("message.general.over-lower", args[0]))
        }
