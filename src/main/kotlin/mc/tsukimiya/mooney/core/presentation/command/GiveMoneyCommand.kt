...
class GiveMoneyCommand : TabExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>?): Boolean {
        ...
        val reason = if (args.size > 2) args[2] else MooneyCore.formatter.formatMessage("log.give", executor)
        try {
            MooneyCore.api.depositMoney(target, amount, reason)
            sender.sendMessage(MooneyCore.formatter.formatMessage("message.givemoney.success", args[0], args[1]))
        } catch (e: MoneyOverUpperLimitException) {
            sender.sendMessage(MooneyCore.formatter.formatMessage("message.general.over-upper", args[0]))
        }
